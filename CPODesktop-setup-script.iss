; Script was developed and maintained by Gokul Ramachandran at Sharp Labs of America.
; All copyright to Sharp Labs of America 2013
...
OutputDir=dist
OutputBaseFilename=CPODesktop-0.2.3-Win32-setup
...
begin
  Result := '';
  sUnInstPath := 'Software\Microsoft\Windows\CurrentVersion\Uninstall\{5D0411E3-47F5-441A-B595-C1DCE87519C2}}_is1'; //Your App GUID/ID
  sUnInstallString := '';
...
var
  ...
  sUnInstallString: string;
begin
  //First stop the application; no matter what
  StopApplication();

  Result := True; // in case when no previous version is found
  if RegValueExists(HKEY_LOCAL_MACHINE,'Software\Microsoft\Windows\CurrentVersion\Uninstall\{5D0411E3-47F5-441A-B595-C1DCE87519C2}}_is1', 'UninstallString') then
  begin
//Your App GUID/ID
    V := MsgBox(ExpandConstant('An old version of the app was detected. Do you want to uninstall it?'), mbInformation, MB_YESNO); //Custom Message if App installed
    if V = IDYES then
    begin
      ...
      MsgBox('Please uninstall the previous version before installing this version', mbInformation, MB_OK)  ;
