...
''
        ...
        sql = "SELECT `Dialog_`, `Control`, `Type`, `X`, `Y`, `Width`, `Height`, `Attributes`, `Property`, `Text`, `Control_Next`, `Help` FROM `Control` WHERE `Dialog_`='ExitDialog' AND `Control`='Finish'";
        view = database.OpenView(sql);
        view.Execute();
        record = view.Fetch();
        record.StringData(11) = "LaunchWizard";
        view.Modify(msiViewModifyReplace, record);
        view.Close();

        // Insert the new CheckBox control
        sql = "INSERT INTO `Control` (`Dialog_`, `Control`, `Type`, `X`, `Y`, `Width`, `Height`, `Attributes`, `Property`, `Text`, `Control_Next`, `Help`) VALUES ('ExitDialog', 'LaunchWizard', 'CheckBox', '15', '150', '320', '12', '3', 'LAUNCHAPP', '{\\VSI_MS_Sans_Serif13.0_0_0}" + checkboxText + "', 'Cancel', '|')";
        view = database.OpenView(sql);
        view.Execute();
        view.Close();
        ...
        sql = "SELECT `Dialog_`, `Control_`, `Event`, `Argument`, `Condition`, `Ordering` FROM `ControlEvent` WHERE `Dialog_`='ExitDialog' AND `Event`='EndDialog'";
        view = database.OpenView(sql);
        view.Execute();
        record = view.Fetch();
        ...
        sql = "INSERT INTO `ControlEvent` (`Dialog_`, `Control_`, `Event`, `Argument`, `Condition`, `Ordering`) VALUES ('ExitDialog', 'Finish', 'DoAction', 'Wizard_Launch', 'LAUNCHAPP=1', '0')";
        view = database.OpenView(sql);
        view.Execute();
        view.Close();
        ...
        sql = "INSERT INTO `CustomAction` (`Action`, `Type`, `Source`, `Target`) VALUES ('Wizard_Launch', '210', '" + fileId + "', '')";
        view = database.OpenView(sql);
        view.Execute();
        view.Close();
        ...
        database.Commit();
        WScript.Echo("Updated all for checkbos control...");
}
catch(e)
{
...
try
{
		...
		WScript.Echo("Updating the SelfReg table...");
        var dllFileId = FindFileIdentifier(database, dllFilename);
        if (dllFileId)
        {
	        sql = "INSERT INTO `SelfReg` (`File_`, `Cost`) VALUES ('" + dllFileId + "', '200')";
	        view = database.OpenView(sql);
	        view.Execute();
	        view.Close();
        }
        ...
        database.Commit();
        WScript.Echo("updated DLL registration...");
}
catch(e)
{
        WScript.StdErr.WriteLine(e);
