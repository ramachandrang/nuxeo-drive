...
void UrlReader::parse()
{
	...
	TCHAR urlParams[MAX_PATH + 50] = TEXT("?state=conflicted&folder=");
...
void longPullThread(void * params){
	...
	TCHAR urlParams[MAX_PATH + 50] = TEXT("?state=conflicted&transition=true&folder=");
...
char* UrlReader::getJsonStringFromServer(const wchar_t* request, bool isThread) {
	...
	hSession = WinHttpOpen( L"WinHTTP - IconOverlayConflict",  
